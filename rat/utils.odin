...
from_polar_deg :: #force_inline proc(speed: f32, angle_degrees: f32) -> [2]f32 {
	angle_radians := math.to_radians_f32(angle_degrees)
	return [2]f32{math.cos(angle_radians) * speed, math.sin(angle_radians) * speed}
}

from_polar_rad :: #force_inline proc(speed: f32, angle_radians: f32) -> [2]f32 {
	return [2]f32{math.cos(angle_radians) * speed, math.sin(angle_radians) * speed}
}

play_sound_with_random_pitch :: proc(sound: rl.Sound, min: f32, max: f32) {
	sound_copy := sound
